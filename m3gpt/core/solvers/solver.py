...
class Solver(object):
    ...
    def __init__(self, C):
        ...
        self.clip_grad_lm = config.get('clip_grad_lm', 0.0)
    ...
    def create_model(self):
        ...
        lm_bn_group_size = config.lm['kwargs'].get('bn_group_size', 1)
        assert lm_bn_group_size == 1, 'other bn group size not support!'
        lm_bn_group_comm = self.ginfo.lm_share_group
        # if lm_bn_group_size == 1:
        #     lm_bn_group_comm = None
        # else:
        #     assert self.C.world_size % lm_bn_group_size == 0
        #     lm_bn_group_comm = simple_group_split(self.C.world_size, self.C.rank, self.C.world_size // lm_bn_group_size)

        ## build lm
        config.lm['kwargs']['bn_group'] = lm_bn_group_comm
        lm_module = lms.lm_entry(config.lm)
        count_parameters_num(lm_module)
        
        # build
        model = lm_model_entry(lm_module)
    ...
    def create_optimizer(self):
        ...
        if self.C.rank == 0:
            self.logger.info('making param_group_lm, num_parameters:{}, args: {}'.format(len(param_group[0]['params']), self.config.optimizer))
    ...
    def load(self, args):
        if args.load_path == '':
            return
        if args.recover:
            self.last_iter = load_state(args.load_path.replace('ckpt_task_', 'ckpt_task{}_'.format(self.ginfo.task_id)), self.model, recover=args.recover)
            self.last_iter -= 1
        else:
            if args.load_single:
    ...
    def pre_run(self):
        ...
        tmp.vlm_grad_norm = AverageMeter(10)
        tmp.lm_grad_norm_list = [torch.Tensor(1).cuda() for _ in range(self.C.world_size)]
    ...
    def gather_result(self):
        ...
        if self.auto_clip:
            vreduce(tmp.vlm_grad_norm, torch.Tensor([tmp.lm_grad_norm/ginfo.task_size]).cuda(), group=ginfo.group)
            vgather(tmp.lm_grad_norm_list, tmp.vlm_grad_norm.avg)
            ...
            vreduce(tmp.vlm_grad_thresh, torch.Tensor([tmp.lm_grad_thresh/ginfo.task_size]).cuda(), group=ginfo.group)
            vgather(tmp.lm_grad_thresh_list, tmp.vlm_grad_thresh.avg)
    ...
    def update(self):
        ...
        if self.clip_grad_lm > 0:
            tmp.lm_grad_norm = torch.nn.utils.clip_grad.clip_grad_norm_(\
                self.model.module.lm_module.parameters(), \
                max_norm=self.clip_grad_lm*(ginfo.task_size**0.5))
            is_inf = np.isinf(tmp.lm_grad_norm)
            is_nan = np.isnan(tmp.lm_grad_norm)
            if ginfo.task_rank == 0 and (is_inf or is_nan):
                self.logger.info('task{} {} lm_grad_norm inf/nan {}/{}'.format(\
    ...
    def tb_logging(self):
        ...
        for tid,ii in enumerate(ginfo.task_root_ranks):
            ...
            if self.clip_grad_lm > 0:
                self.tb_logger.add_scalar('lm_grad_norm_{}'.format(ginfo.task_names[tid]), tmp.lm_grad_norm_list[ii], tmp.current_step)
            if self.clip_grad_neck > 0:
