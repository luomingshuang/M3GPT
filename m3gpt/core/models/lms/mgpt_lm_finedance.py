...
class MLM(nn.Module):
    ...
    def forward_encdec(
        ...
        if texts is not None and tasks[0]['class'] == 'm2m':
            texts_final = texts
        ...
        if audio_tokens is not None and tasks[0]['class'] == 'm2m':
            texts_final = self.audio_token_to_string(audio_tokens, a_lengths)
        ...
        with torch.no_grad():
            ## debug for d2a
            print('inputs: ', source_input_ids[:, :50])
            ...
            print('pd_gen: ', outputs[0][:50]-32615)
