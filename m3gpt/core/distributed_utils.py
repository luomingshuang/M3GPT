...
class DistModule(torch.nn.Module):
    def __init__(self, module, sync=False, task_grp=None, share_lm_group=None, ignore_bcast=None, \
                 task_weight=None, task_size=None):
        super(DistModule, self).__init__()
        self.module = module
        self.sync = sync
        self.task_grp = task_grp
        self.share_lm_group = share_lm_group
        ...
        broadcast_params_multitask(self, self.task_grp, self.share_lm_group, ignore_bcast)
    ...
    def reduce_gradients(self, task_specific=False):
        if self.sync:
            if not task_specific:
                if self.task_grp is not None or self.share_lm_group is not None:
                    for name, param in self.named_parameters():
                        ...
                        elif param.lm_specific and param.requires_grad:
                            allreduce(param.grad.data, group=self.share_lm_group)
...
def broadcast_params_multitask(model, task_grp, share_lm_group, ignore):
    """ broadcast multi-task model parameters """
    if task_grp is not None or share_lm_group is not None \
            or share_lm_group is not None or share_lm_group is None:

        for name, p in model.named_parameters():
            if ignore and name in ignore:
                printlog('param {} ignored in broadcast'.format(name))
                continue
            assert p.task_specific + p.lm_specific <= 1.5, \
                "param could not be task_specific, lm_specific at same time"
            try:
                if p.task_specific:
                    # printlog(f'broadcasting task-specific param {name}\tgroup={task_grp}')
                    broadcast(p, _get_global_rank(task_grp, 0), group=task_grp)
                elif p.lm_specific:
                    # import pdb; pdb.set_trace()
                    # printlog(f'broadcasting lm-specific param {name}\tgroup={share_lm_group}')
                    broadcast(p, _get_global_rank(share_lm_group, 0), group=share_lm_group)
                else:
                    # printlog(f'broadcasting non-specific param {name}')
                    broadcast(p, 0)
            except Exception as e:
                printlog(f"****===>> p.task_specific: {p.task_specific}  "
                      f"p.lm_specific: {p.lm_specific}")
                printlog(f'error:{e}')
                raise RuntimeError(
                    'param {} does not have task_specific or lm_specific'.format(name))

        for name, b in model.named_buffers():
            if ignore and name in ignore:
                printlog('buffer {} ignored in broadcast'.format(name))
                continue
            assert b.task_specific + b.lm_specific <= 1, \
                "buffer could not be task_specific, lm_specific at same time"
            try:
                if b.task_specific:
                    printlog('broadcasting task-specific buffer {}'.format(name))
                    broadcast(b, _get_global_rank(task_grp, 0), group=task_grp)
                elif b.lm_specific:
                    printlog('broadcasting lm-specific buffer {}'.format(name))
                    broadcast(b, _get_global_rank(share_lm_group, 0), group=share_lm_group)
                else:
                    broadcast(b, 0)
            except:
...
def get_local_rank_no_slurm():
    return int(os.environ["RANK"])

def get_world_size_no_slurm():
    return int(os.environ["WORLD_SIZE"])

def dist_init(method='slurm', port='5671', gpus='0'):
    # assert method == 'slurm'
    ...
    if method == 'no_slurm':
        ...
        rank = get_local_rank_no_slurm()
        world_size = get_world_size_no_slurm()
        # import pdb; pdb.set_trace()
...
class DistributedGivenIterationSampler(Sampler):
    ...
    def gen_s5(self):
        ...
        num_class = len(keys)
        printlog('class map done.')
        printlog('{} class with {} samples in a batch!'.format(self.batch_size // self.imageNumPerClass,
                                                               self.imageNumPerClass))
    ...
    def gen_new_list(self):
        ...
        # each process shuffle all list with same seed, and pick one piece according to rank
            ...
            indices = indices[:all_size]
            printlog(
                'using strategy 3, mean_num: {}, origin_len: {}, balanced_len: {}'.format(mean_num, len(self.dataset),
                                                                                          len(indices)))
            ...
            for pid, idxs in class2id.items():
                ...
                mini_indices.extend(
                    [idxs[i:i + self.imageNumPerClass] for i in range(0, len(idxs), self.imageNumPerClass)])
...
class RandomIdentitySampler(Sampler):
    ...
    def __fetch_current_node_idxs(self, final_idxs, length):
        ...
        for i in range(0, block_num * self.world_size, self.world_size):
            index = range(self.batch_size * self.rank + self.batch_size * i,
                          min(self.batch_size * self.rank + self.batch_size * (i + 1), total_num))
...
class DistributedGroupSampler(Sampler):
    ...
    def __init__(self,
        ...
        self.flag = np.tile(dataset_flag, (total_iter - 1) * batch_size // len(self.dataset) + 1)[
...
def vgather(x_list, x):
    dist.gather(torch.Tensor([x]).cuda(), x_list if get_rank() == 0 else None)
