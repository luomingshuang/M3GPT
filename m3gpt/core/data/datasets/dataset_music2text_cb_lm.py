...
class Music2TextDatasetCB(data.Dataset):
    def __init__(
        self,
        ginfo,
        data_root,
        ...
        **kwargs,
        ...
        self.finedance_text_path = finedance_text_path
    ...
    def load_audio_dance_finedance(self, split_file, dance_dir, audio_dir):
        ...
        for _, name in enumerate(track(id_list)):
            try:
                ...
                finedance_label_json_file = pjoin(self.finedance_text_path, f'{name}.json')
                style = json.load(open(finedance_label_json_file, 'r'))["style2"]
                caption = f"A person is dancing {style}."
                data_dict[name] = {
                    'm_token': m_token,
                    'a_token': a_token,
                    'caption': caption
    ...
    def __getitem__(self, item):
        ...
        m_tokens, a_tokens, caption = data['m_token'], data['a_token'], data['caption']

        ## setting seconds for training
        ## seconds = 10 # random.randint(6, 10)
        win_motion_size = self.win_motion_size
        win_audio_size = self.win_audio_size
        if win_motion_size < m_tokens.shape[0]:
            idx = random.randint(0, m_tokens.shape[0] - win_motion_size)
            dance_idx = int(idx * self.stride)
        
            m_tokens = m_tokens[idx: idx + win_motion_size]
        ...
        return caption, a_tokens, None, None, a_tokens_len, task