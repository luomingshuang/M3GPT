...
class Music2DanceDatasetCB_DA2D(data.Dataset):
    def __init__(
        self,
        ginfo,
        data_root,
        ...
        **kwargs,
        ...
        if only_aistpp:
            ...
            name_list = aistpp_name_list
        if only_aistpp and only_finedance:
            # finedance Data path
            finedance_split_file = pjoin(data_root, 'splits/finedance/{}.txt'.format(split))
            finedance_dance_cb_dir = finedance_dance_code_path
            finedance_audio_cb_dir = finedance_music_code_path
    ...
    def load_audio_dance_finedance(self, split_file, dance_dir, audio_dir):
        ...
        for _, name in enumerate(track(id_list)):
            try:
                ...
                # print(m_tokens.shape[0])
                if m_token.shape[0] > 1+2*int(self.motion_tokens_per_second * self.seconds):
                    data_dict[name] = {
                        'm_token': m_token,
                        'a_token': a_token,
                    }
    ...
    def __getitem__(self, item):
        ...
        win_motion_size = int(self.motion_tokens_per_second * self.seconds)
        win_audio_size = int(self.audio_tokens_per_second * self.seconds)

        motion_start_idx = win_motion_size
        audio_start_idx = win_audio_size

        motion_end_idx = m_tokens.shape[0] - win_motion_size

        # if win_motion_size < m_tokens.shape[0]:
        
        idx = random.randint(win_motion_size, m_tokens.shape[0] - win_motion_size)
        dance_idx = int(idx * self.stride)
    
        m_tokens = m_tokens[(idx - win_motion_size) : (idx + win_motion_size)]
        a_tokens = a_tokens[dance_idx: dance_idx + win_audio_size]

        m_tokens_len = win_motion_size
        a_tokens_len = win_audio_size
        # else:
