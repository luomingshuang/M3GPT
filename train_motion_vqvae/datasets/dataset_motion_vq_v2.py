...
class MotionDatasetVQ_V2(data.Dataset):
    def __init__(
        self, 
        data_root, 
        split,
        ...
        **kwargs,      
                    ...
                    if motion.shape[0] >= 640:
                        ...
                        for i in range(num_clips):
                            ...
                            if motion_clip.shape[0] > self.window_size:
                                ...
                                self.lengths.append(motion_clip.shape[0])
        ...
        assert len(self.name_list) == len(self.length_arr)
        assert len(self.name_list) == len(self.motion_dict)
    ...
    def __getitem__(self, item):
        ...
        name = self.name_list[audio_id]
        data = self.motion_dict[name]
...
def prepare_motion_vqv2_data():
    motion_vq_dataset_train = MotionDatasetVQ_V2(
    ...
    motion_vq_dataset_val = MotionDatasetVQ_V2(
        data_root=dataset_opt.data_root,
        split='val',
        window_size=dataset_opt.window_size,
        mean=dataset_opt.mean,
    ...
    motion_vq_dataset_test = MotionDatasetVQ_V2(
        data_root=dataset_opt.data_root,
        split='test',
        window_size=dataset_opt.window_size,
        mean=dataset_opt.mean,
...
def prepare_motion_vqv2_data_test_motion():
    motion_vq_dataset_val = MotionDatasetVQ_V2(
        data_root=dataset_opt.data_root,
        split='val',
        window_size=dataset_opt.window_size,
