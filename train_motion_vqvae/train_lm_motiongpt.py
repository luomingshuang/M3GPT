...
from datasets.dataset_motion_vq import prepare_motion_vq_data
...
def main():
    ...
    ## loading motion data
    motion_vq_dataset_train, motion_vq_dataset_val, motion_vq_dataset_test = prepare_motion_vq_data()
    ...
    for epoch in range(1, opt.max_epoch):
        ...
        if opt.distributed:
            if torch.distributed.get_rank() == 0:
                ...
                torch.save(model.state_dict(), "{}/unified_vq_vae_motion.pt".format(opt.vq_vae_motion_pretrained_dir))
                if epoch % 100000 == 0:
                    torch.save(model.state_dict(), "{}/unified_vq_vae_motion_{}.pt".format(opt.vq_vae_motion_pretrained_dir, epoch))
        else:
            torch.save(model.state_dict(), "{}/unified_vq_vae_motion.pt".format(opt.vq_vae_motion_pretrained_dir))
            if epoch % 100000 == 0:
                torch.save(model.state_dict(), "{}/unified_vq_vae_motion_{}.pt".format(opt.vq_vae_motion_pretrained_dir, epoch))
