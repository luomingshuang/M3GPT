...
def main():
    ...
    with torch.no_grad():
        ...
        for batch_idx, batch in enumerate(tqdm(train_dataloader)):
            ...
            if 'motionx' in name:
                name = name.replace('motionx_new_joint_vecs', 'motionx_vq_vae_codes')
            if 'finedance' in name:
                name = name.replace('finedance_new_joint_vecs', 'finedance_vq_vae_codes')
            if 'aistpp' in name:
                name = name.replace('aistpp_new_joint_vecs', 'aistpp_vq_vae_codes_new')
            ...
            np.save(name+'.npy', code_idxs[0].detach().cpu().numpy())
        ...
        for batch_idx, batch in enumerate(tqdm(test_dataloader)):
            ...
            ## motion encode feats to codes idx
            code_idxs, _ = model.module.encode(feats_ref)
            print(feats_ref.shape, code_idxs[0].shape, feats_ref.shape[1]/30)
            np.save(name+'.npy', code_idxs[0].detach().cpu().numpy())
