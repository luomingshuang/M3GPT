...
def main():
    ...
    state_dict = torch.load("{}/unified_vq_vae_motion_20240306.pt".format(opt.vq_vae_motion_pretrained_dir), map_location='cpu')
    ...
    output_val_dir_pd = '/home/luomingshuang/codes/multi-modal-motion-generation/unihcp-for-unified-motion-tasks/train_motion-vqvae/visualization_samples/val_gt_ori'
    output_val_dir_gt = '/home/luomingshuang/codes/multi-modal-motion-generation/unihcp-for-unified-motion-tasks/train_motion-vqvae/visualization_samples/val_pd_ori'
    ...
    for batch_idx, batch in enumerate(tqdm(val_dataloader)):
        ...
        if batch_idx <= 2:
            ...
            # np.save(os.path.join(output_val_dir_pd, f'aistpp_{batch_idx}.npy'), rs_set['joints_rst'].detach().cpu().numpy()[0])
            # np.save(os.path.join(output_val_dir_gt, f'aistpp_{batch_idx}.npy'), rs_set['joints_ref'].detach().cpu().numpy()[0])
