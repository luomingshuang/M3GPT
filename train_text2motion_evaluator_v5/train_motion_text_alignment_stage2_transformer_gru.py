## How to run: 
## CUDA_VISIBLE_DEVICES='0' torchrun --nproc_per_node=1 --master_port=29999 train_motion_text_alignment_stage2_transformer_gru.py 
...
def main():
    ...
    if check_print_rank(args):
        ...
        tb_writer = SummaryWriter(log_dir="tensorboard_20240511/log-train-{}".format(start_time))
    else:
        tb_writer = None
    ...
    for epoch in range(0, opt.max_epoch):
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            texts = batch['texts']
            m_masks = batch['motion_masks'].to(device)
            
            m_avg, t_avg = model(m_feats, texts, m_masks)
            optimizer.zero_grad()
        ...
        if opt.distributed:
            if torch.distributed.get_rank() == 0:
                ...
                torch.save(model.state_dict(), "{}/pretrained_motion_text_alignment_512_movement_bigruco_v6.pt".format(opt.motion_text_alignment_pretrained_dir))
                # if epoch % 50 == 0:
